...
# Arvin Castro
# December 17, 2010
# http://codecri.me/sources/includes/class-xhttp-php/plugin-xhttp-oauth-php/
...
class xhttp_oauth {
	...
	public static function oauth_sign_request(&$urlparts, &$requestData) {
		...
		if((isset($requestData['headers']['Content-Type']) AND $requestData['headers']['Content-Type']) == 'multipart/form-data' OR (isset($requestData['files']) AND count($requestData['files']))) {
			$filtered_data = array();
			foreach($oauth_data as $key => $value) {
				if(strpos($key, "oauth_") === 0) $filtered_data[$key] = $value;
			}
			$oauth_data = $filtered_data;
		}
		...
		if(!isset($requestData['headers']['User-Agent']))
			$requestData['headers']['User-Agent']  = 'codecri.me xhttp oauth plugin';
